...
namespace NCloud.FileProviders.Support
{
    ...
    public class HttpRemoteFileInfo : FileInfoDecorator, IRemoteFileInfo, IRandomAccessFileInfo
    {
        ...
        public HttpRemoteFileInfo(string remoteUrl, IFileInfo fileInfo, HttpClient client, bool checkLength = false, bool supportRange = true) : base(fileInfo)
        {
        ...
        {
            if (!supportRange && !(InnerIFileInfo is IRandomAccessFileInfo))
            {
                throw new IOException("not support random access");
            }
            if (InnerIFileInfo is IRandomAccessFileInfo randomAccessFileInfo)
            {
